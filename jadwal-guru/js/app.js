const supabaseUrl = 'https://ruffztvoudvyldjtziwo.supabase.co';
const supabaseKey = 'sb_publishable_i1fRGhJ8Asz8rs-l0oD_IA_c5SADpGL';
...
async function fetchSchedulesFromDB() {
  const { data, error } = await db.from('jadwal').select('*');
  if (error) {
  ...
  const formatted = {};
  data.forEach(item => {
    if (!formatted[item.guru_id]) formatted[item.guru_id] = [];
    formatted[item.guru_id].push(item);
  });
  return formatted;
}
...
async function submitJadwalForm(ev) {
  ev.preventDefault();
  ...
  const guru_id = state.currentGuru.id;

  if (state.editingId) {
    const { error } = await db.from('jadwal').update({ hari, mapel, kelas, mulai, selesai, ruang }).eq('id', state.editingId);
    if (!error) {
    ...
    const newEntry = { id: newId, guru_id, hari, mapel, kelas, mulai, selesai, ruang };
    const { error } = await db.from('jadwal').insert([newEntry]);
    if (!error) list.push(newEntry);
  }
  
  closeModal(modalJadwal);
  state.activeDay = hari;
  renderWeeklyHours(); renderDayTabs(); renderDayPanel(); renderReminder();
  toast(conflict ? `${t("toastSaved")} ${t("conflictWarning")} "${conflict.mapel}" (${conflict.mulai}–${conflict.selesai})` : t("toastSaved"));
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  state.schedules = await fetchSchedulesFromDB();
